...
/* -----------------------Non Carrier Summary Stats----------------------- */
data noncarriers;
	set HEMOPHIL;
	if Group^=1 then delete;
run;
proc means data=noncarriers n mean max min range std missing noprint fw=4;
	class group ;
	var Activity;
	output out=NonCarriersActivitySummary mean=mean std=std;
	title 'Non Carriers Activity Summary';
run;
proc means data=noncarriers n mean max min range std missing noprint fw=4;
	class group ;
	var Antigen;
	output out=NonCarriersAntigenSummary mean=mean std=std;
	title 'Non Carriers Antigen Summary';
run;
data NonCarriersActivitySummary;
	set NonCarriersActivitySummary;
	if _type_^=1 then delete;
run;
data NonCarriersAntigenSummary;
	set NonCarriersAntigenSummary;
	if _type_^=1 then delete;
run;
...
run;
proc means data=carriers n mean max min range std missing noprint fw=4;
	class group ;
	var Activity;
	output out=carriersActivitySummary mean=mean std=std;
	title 'Carriers Activity Summary';
run;
proc means data=carriers n mean max min range std missing noprint fw=4;
	class group ;
	var Antigen;
	output out=carriersAntigenSummary mean=mean std=std;
	title 'Non Carriers Antigen Summary';
run;
data carriersActivitySummary;
	set carriersActivitySummary;
	if _type_^=1 then delete;
run;
data carriersAntigenSummary;
	set carriersAntigenSummary;
	if _type_^=1 then delete;
run;

/* Look at Diagnostics for NonCarriers*/
proc glm data= WORK.noncarriers  PLOTS=(DIAGNOSTICS RESIDUALS);
class Group;
...
run;
quit;
/* proc sgscatter data = noncarriers; */
/* plot Activity*Antigen; */
/* title 'Non Carriers Scatter'; */
/*  */
/* proc sgscatter data = carriers; */
/* plot Activity*Antigen; */
/* title 'Carriers Scatter'; */
