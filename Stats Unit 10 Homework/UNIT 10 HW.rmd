...
```{r}
MaleTest<-lm(Tcell~Mass,data = Male_Display_Data_Set)
#qt(0.995, 198)
abs(qt(0.01/2, 198))
summary(MaleTest)
confint(MaleTest, level = .99)
plot(Male_Display_Data_Set$Mass,Male_Display_Data_Set$Tcell)
#,xlim = c(0,220), ylim = c(0,605),xlab = "Budget",ylab = "Gross", main = "Gross Sales versus Budget"
plot(Male_Display_Data_Set$Mass)
```
```{r}
movies <- read_csv("~/GitHub/StatisticalFoundations/Stats Unit 10 Homework/MovieData.csv")
movies
movies.lm=lm(gross~budget, data = movies)
newx=movies$budget
newx=sort(newx)
prd_c=predict(movies.lm, newdata= data.frame(budget = newx), interval=c("confidence"), type = c("response"), level=0.95) 
newpoint <- data.frame(gross=NA, budget=95)
predict(movies.lm, newpoint, interval="confidence", level = 0.95)
prd_p=predict(movies.lm, newdata= data.frame(budget = newx), interval=c("prediction"), type = c("response"), level=0.95) 
predict(movies.lm, newpoint, interval="prediction", level = 0.95)
#Plot with confidence and prediction intervals
library(ggplot2)
plot(movies[,1],xlim = c(0,220), ylim = c(0,605),xlab = "Budget",ylab = "Gross", main = "Gross Sales versus Budget")
